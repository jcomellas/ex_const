defmodule Const do
  ...
  @doc """
  Determine if a quoted expression corresponds to a constant value.
  """
  @spec quoted_const?(term) :: boolean
  def quoted_const?(value) when is_atom(value) or is_number(value) or is_binary(value) do
    # Booleans are also considered atoms.
    true
  end
  def quoted_const?(value) when is_list(value) do
    quoted_const_list?(value)
  end

  def quoted_const?({name, _metadata, args}) when (name == :{} or name == :%{}) and is_list(args) do
    # Tuples and maps in quoted format.
    quoted_const_keyword?(args)
  end
  def quoted_const?({name, metadata, _args}) when is_atom(name) and is_list(metadata) do
    # Reference to a variable or function.
    false
  end
  def quoted_const?(value) when is_tuple(value) do
    quoted_const_tuple?(value)
  end
  def quoted_const?(_value) do
    # Ports, pids and references are not considered constants.
    false
  end

  @doc "Determine if the elements of a quoted list expression are constant."
  def quoted_const_list?([value | tail]) do
    if quoted_const?(value) do
      quoted_const_list?(tail)
    else
      false
    end
  end
  def quoted_const_list?([]) do
    true
  end

  @doc "Determine if the elements of a quoted tuple expression are constant."
  def quoted_const_tuple?(tuple, index \\ 0)

  def quoted_const_tuple?(tuple, index) when index < tuple_size(tuple) do
    if quoted_const?(elem(tuple, index)) do
      quoted_const_tuple?(tuple, index + 1)
    else
      false
    end
  end
  def quoted_const_tuple?(_tuple, _index) do
    true
  end

  @doc "Determine if the elements of a quoted map expression are constant."
  def quoted_const_keyword?([{key, value} | tail]) do
    if quoted_const?(key) and quoted_const?(value) do
      quoted_const_keyword?(tail)
    else
      false
    end
  end
  def quoted_const_keyword?([]) do
    true
  end
end
